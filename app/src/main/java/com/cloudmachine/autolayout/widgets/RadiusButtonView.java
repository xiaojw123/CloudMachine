...
public class RadiusButtonView extends AutoRelativeLayout {
    ...
    private int roundRadius = (int) getResources().getDimension(R.dimen.radius_button_roundRadius);
    ;// 5dp 圆角半径
...
//        addView(mContainer, lp);
    ...
    public void setButtonClickEnable(boolean flag) {
        isClickEnable = flag;
        if (isClickEnable) {
            radius_button_text.setTextColor(getResources().getColor(R.color.cor15));
        } else {
            radius_button_text.setTextColor(getResources().getColor(R.color.cor2015));
        }
    }

    public void setButtonClickListener(final OnClickListener listener) {
        radius_button_text.setOnClickListener(new OnClickListener() {
            @Override
