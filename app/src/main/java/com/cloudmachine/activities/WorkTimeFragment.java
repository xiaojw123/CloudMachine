...
public class WorkTimeFragment extends Fragment implements View.OnClickListener,Callback, SeekBar.OnSeekBarChangeListener,
   ...
   // private Handler mHandler;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
      super.onCreateView(inflater, container, savedInstanceState);
        ...
        if (null != viewParent) {
            ...
            viewParent = inflater.inflate(R.layout.fragment_work_time, null);
    ...
    private void initView() {
        ...
        deviceId = workHoursActivity.getDeviceId();
        new GetWorkTimeListAsync(deviceId, mContext, mHandler).execute();

        daily_layout = viewParent.findViewById(R.id.daily_layout);
        daily_title = (TextView) viewParent.findViewById(R.id.daily_title);
...
//	        barr.setData(data)
        ...
        if(page == 0){
            ...
            for(int i=0; i<len; i++){
                if(i == len -1){
                    nColor[i] = getResources().getColor(R.color.chart_yes_dw);
                }else{
                    nColor[i] = getResources().getColor(R.color.chart_yes1);
                }
            }
            set1.setColors(nColor);
        }else{
            set1.setColor(getResources().getColor(R.color.chart_yes1));
        }
...
//	        mChart.setVisibleXRangeMaximum(7);  //一个界面显示多少个点，其他点可以通过滑动看到
    ...
    private void initDailyChart(){
        dailyChart = (HorizontalBarChart) viewParent.findViewById(R.id.dailyChart);
...
//		setDailyData();
        ...
        new DailyWorkDitailsListAsync(deviceId,date,mContext,mHandler).execute();
