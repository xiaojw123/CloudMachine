...
public class DeleteFenceAsync extends ATask {

	private Context context;
	private Handler handler;
	private int type;
	
	public DeleteFenceAsync(Context context,Handler handler){
	...
	protected String doInBackground(String... params) {
		...
		String result = null;
		try {
				result = httpRequest.post(Constants.URL_DELETEFENCE, list);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	protected void decodeJson(String result) {
		...
		if (isSuccess) {
			try{
				...
				BaseBO baseBo = gson.fromJson(result,
						BaseBO.class);
				msg.what = Constants.HANDLER_DELETEFENCE_SUCCESS;
		...
		msg.what = Constants.HANDLER_DELETEFENCE_FAIL;
		msg.obj = message;
		handler.sendMessage(msg);
	}
