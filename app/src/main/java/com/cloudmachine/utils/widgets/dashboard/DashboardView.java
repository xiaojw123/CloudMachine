package com.cloudmachine.utils.widgets.dashboard;
...
public class DashboardView extends View {
    ...
    private int mArcBgColor = Color.rgb(0xe9, 0xe9, 0xe9); // 弧度背景色
    private int mArcUpColor = Color.rgb(0xf8, 0x60, 0x30); // 弧度上色
    ...
    public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBigSliceCount = a.getInteger(R.styleable.DashboardView_bigSliceCount, 1);
        ...
        mPointerRadius = a.getDimensionPixelSize(R.styleable.DashboardView_pointerRadius, mRadius / 5 * 4);
...
//        mBigSliceRadius = mSmallSliceRadius - dpToPx(8);
        mBigSliceRadius = mRadius - mStripeWidth;
        ...
        initArcAngle = getArcAngleFromResult(mRealTimeValue);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        {
      //绘制圆弧
        mPaintStripe.setColor(mArcBgColor);//圆弧背景颜色
        canvas.drawArc(mRectStripe, mStartAngle,
        		mSweepAngle, false, mPaintStripe);//背景圆弧
        mPaintStripe.setColor(mArcUpColor);//圆弧上色
        canvas.drawArc(mRectStripe, mStartAngle,
        		initArcAngle, false, mPaintStripe);//圆弧值
        //绘制三角形指针
       ...
       /* mPaintArc.setStrokeWidth(dpToPx(2));
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            ...
            if (mStripeMode == StripeMode.NORMAL && mStripeHighlight != null) {
            	...
            	if(i == 0){
            		if(mRealTimeValue<=0){
            			mPaintArc.setColor(mArcBgColor);
            		}else{
            			mPaintArc.setColor(mArcUpColor);
            		}
            	}else if(i == mBigSliceCount){
...
//                mCenterY + mCircleRadius + dpToPx(2) + dpToPx(25), mPaintValue);
    ...
    private float getArcAngleFromResult(float result) {
    	...
    	if(result>mMaxValue)
    		return mSweepAngle;
    	return result/mMaxValue*mSweepAngle;
    }
    ...
    public void animate(long m, float realTimeValue){
    	if(realTimeValue<mMinValue )
    		realTimeValue = mMinValue;
    	if(realTimeValue>mMaxValue)
    		realTimeValue = mMaxValue;
    	
    	ValueAnimator valueAnimator = ValueAnimator.ofFloat(0, realTimeValue);
