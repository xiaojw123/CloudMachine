...
public class PanelBomView extends LinearLayout implements GestureDetector.OnGestureListener{  
  
    GestureDetector mGesture = null;  
    private boolean isScrolling = false;  
    private int MAX_HEIGHT = 800;//拖动的最大高度,当前布局位于父布局下面-80位置，这个仅仅是调试参数，这个变量是动态设置的。  
    private float mScrollX; // 滑块滑动距离  
    public PanelBomView(Context context) {  
        super(context);  
        init();  
    }  
    public PanelBomView(Context context, AttributeSet attrs) {  
        super(context, attrs);  
        init();  
    }  
    //初始化一些参数  
