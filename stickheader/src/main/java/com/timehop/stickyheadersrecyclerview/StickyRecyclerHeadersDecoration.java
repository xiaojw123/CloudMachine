...
public class StickyRecyclerHeadersDecoration extends RecyclerView.ItemDecoration {
  ...
  public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
    ...
    if (mHeaderPositionCalculator.hasNewHeader(itemPosition, mOrientationProvider.isReverseLayout(parent),parent.getChildCount())) {
      Log.d(TAG,"getItemOffsets newHeader position____"+itemPosition);
  ...
  public void onDrawOver(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
    ...
    Log.d(TAG, "onDrawOver: childCount___"+childCount);
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      Log.d(TAG, "onDrawOver: itemView___"+itemView+" , position__"+position);
      if (position == RecyclerView.NO_POSITION) {
          continue;
      }
      boolean hasStickyHeader = mHeaderPositionCalculator.hasStickyHeader(itemView, mOrientationProvider.getOrientation(parent), position,childCount);
      if (hasStickyHeader || mHeaderPositionCalculator.hasNewHeader(position, mOrientationProvider.isReverseLayout(parent),childCount)) {
        Log.d("test","onDrawOver newHeader position____"+position);
  ...
  public View getHeaderView(RecyclerView parent, int position) {
    return mHeaderProvider.getHeader(parent, Math.max(0,position));
  }
