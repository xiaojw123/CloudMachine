...
import com.yanzhenjie.recyclerview.swipe.widget.DefaultRefreshHeader;
import com.yanzhenjie.recyclerview.swipe.widget.BaseRefreshHeader;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (lm instanceof LinearLayoutManager) {
                    int fos = ((LinearLayoutManager) lm).findFirstVisibleItemPosition();
                    if (fos == 0) {
                        if (mRefreshView != null) {
                            final float deltaY = e.getRawY() - mLastY;
                            mLastY = e.getRawY();
                            mRefreshView.onMove(deltaY / DRAG_RATE);
                            if (mRefreshView.getVisibleHeight() > 0 && mRefreshView.getState() < DefaultRefreshHeader.STATE_REFRESHING) {
                                return false;
                            }
                        }
                    }
...
//                break;
    ...
    private DefaultRefreshHeader mRefreshView;
    ...
    public void useDefaultRefresh() {
        if (mRefreshView == null) {
            mRefreshView = new DefaultRefreshHeader(getContext());
            mRefreshView.setProgressStyle(DefaultRefreshHeader.BallSpinFadeLoader);
