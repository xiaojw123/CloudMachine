 ...
 * @date 2013-10-08
 */
public class InnerXListView extends ListView implements OnScrollListener {
//    private static final String TAG = "XListView";

	private float mLastY = -1; // save event y
    private float mInnerLastY = -1; // save event y
    ...
    private boolean isScrollViewStart,isScrollViewEnd;
    ...
    public void setParentScrollView(ScrollView parentScrollView) {
        ...
        this.parentScrollView.setOnTouchListener(new TouchListenerImpl());
    }
    ...
    public InnerXListView(Context context) {
        super(context);
        initWithContext(context);
    }

    public InnerXListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initWithContext(context);
    }

    public InnerXListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
            	final float deltaScrollY = mInnerLastY - ev.getRawY();
           	 	mInnerLastY = ev.getRawY();
            	float scrollY = parentScrollView.getScrollY();
            	if (getFirstVisiblePosition() == 0){
               	 	parentScrollView.scrollTo(0, (int)(scrollY+deltaScrollY));
            	}
	 ...
	 private class TouchListenerImpl implements OnTouchListener{
	        ...
	        public boolean onTouch(View view, MotionEvent motionEvent) {
	            ...
	            case MotionEvent.ACTION_MOVE:
	                 ...
	                 int scrollViewMeasuredHeight=parentScrollView.getChildAt(0).getMeasuredHeight();
	                 if(scrollY==0){
	                	 isScrollViewStart = true;
	                	 isScrollViewEnd = false;
	                    }else
	                 if((scrollY+height)==scrollViewMeasuredHeight){
	                	 isScrollViewStart = false;
	                	 isScrollViewEnd = true;
	                    }else{
	                    	isScrollViewStart = false;
		                	 isScrollViewEnd = false;
	                    }
